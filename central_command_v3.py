"""
central_command_v3.py
---------------------
Central Command V3 — Maximum Returns, Tighter Exit Filter.
 
Changes from CC2:
  - Requires ALL 3 signals to trigger before exiting (was 2/3)
  - Fewer false exits = more time in V5 = higher returns
  - Cooldown reduced to 10 days = faster re-entry after crash clears
  - Emergency DD kept at 20%
 
Target: +170-200% return, -15 to -20% max drawdown
 
Usage:
    python central_command_v3.py
"""
...
MA_WINDOW          = 20
SIGNALS_TO_EXIT    = 3     # ALL 3 must trigger (was 2 in CC2)
SIGNALS_TO_REENTER = 0     # all must clear
COOLDOWN_DAYS      = 10    # faster re-entry (was 20 in CC2)
EMERGENCY_DD       = 0.20
# ──────────────────────────────────────────────────────────────────────────────
...
def run_cc3():
    print(f"\nCentral Command V3 ({BACKTEST_START} to {BACKTEST_END})...")
    all_data  = load_data()
    all_dates = sorted(all_data["Date"].unique())
    portfolio = CC3Portfolio()
    top_syms  = get_top_n_symbols(all_data, INDEX_TOP_N)
 
    print(f"Backtesting 48 stocks across {len(all_dates)} days...")
    print(f"  Strategy: V5 passive + early warning (ALL 3 signals required)")
    print(f"  Cooldown: {COOLDOWN_DAYS}d | Emergency DD: {EMERGENCY_DD*100:.0f}%\n")
    ...
    for day_idx, date in enumerate(all_dates):
        ...
        if day_idx % 5 == 0 and day_idx > VOL_WINDOW + MOMENTUM_WINDOW:
            n_signals, signal_details = compute_early_warning(all_data, top_syms, date)
        ...
        if portfolio.mode == "V5" and dd >= EMERGENCY_DD:
...
def print_report(metrics, portfolio):
    print("\n" + "═" * 58)
    print("  CENTRAL COMMAND V3 — ALL 3 SIGNALS REQUIRED")
    ...
    print(f"  {'─'*42}")
    print(f"  CC3 Max Drawdown    : {metrics['max_drawdown']:>+11.2f}%")
    ...
    print("=" * 58)
    print("\n  ── Full Comparison ──────────────────────────────────────")
    print(f"  {'Metric':<22} {'V4':>8} {'CC3':>8} {'CC2':>8} {'V6':>8} {'V5':>8} {'Bench':>8}")
    print(f"  {'─'*68}")
    print(f"  {'Total Return':<22} {'+66.34%':>8} {metrics['total_return']:>+7.2f}% {'+145.21%':>8} {'+110.40%':>8} {'+204.47%':>8} {'+176.48%':>8}")
    print(f"  {'Max Drawdown':<22} {'-16.19%':>8} {metrics['max_drawdown']:>+7.2f}% {'-11.19%':>8} {'-21.74%':>8} {'-40.63%':>8} {'-34.82%':>8}")
    print("=" * 58)
 
    pd.DataFrame(portfolio.equity_curve).to_csv("data/equity_curve_cc3.csv", index=False)
    print(f"\n  Equity curve → data/equity_curve_cc3.csv")
    if portfolio.trade_log:
        pd.DataFrame(portfolio.trade_log).to_csv("data/trade_log_cc3.csv", index=False)
        print(f"  Trade log    → data/trade_log_cc3.csv")
    if portfolio.mode_log:
        pd.DataFrame(portfolio.mode_log).to_csv("data/mode_log_cc3.csv", index=False)
        print(f"  Mode log     → data/mode_log_cc3.csv")

"""
portfolio_allocator_v2.py
-------------------------
Portfolio Allocator V2 — Dual Database Architecture.

Capital allocation:
  - 40% → V5 Passive top 100 Nifty 500 (nifty500.db) — max returns
  - 40% → V4 Active Nifty 50 (nifty50.db) — crisis protection + alpha
  - 20% → CC2 Early Warning on Nifty 500 (nifty500.db) — risk adjusted

V4 runs on its home turf — Nifty 50 large caps where it actually works.
V5 and CC2 capture mid/small cap growth on Nifty 500.

Usage:
    python portfolio_allocator_v2.py
"""
...
# ── TWO DATABASES ─────────────────────────────────────────────────────────────
DB_PATH_50  = "data/nifty50.db"    # V4 active system
DB_PATH_500 = "data/nifty500.db"   # V5 passive + CC2
# ──────────────────────────────────────────────────────────────────────────────
...
# ── CAPITAL ALLOCATION ────────────────────────────────────────────────────────
V5_FRACTION  = 0.80   # passive Nifty 500 top 100
V4_FRACTION  = 0.20   # active Nifty 50
CC2_FRACTION = 0.00001   # early warning Nifty 500
# ──────────────────────────────────────────────────────────────────────────────
...
def compute_early_warning(data500, top_syms, date):
    hist       = data500[data500["Date"] <= date]
...
def run_allocator():
    print(f"\nPortfolio Allocator V2 — Dual Database")
    print(f"  {BACKTEST_START} to {BACKTEST_END}")
    print(f"  V5 {V5_FRACTION*100:.0f}% Nifty500 | V4 {V4_FRACTION*100:.0f}% Nifty50 | CC2 {CC2_FRACTION*100:.0f}% Nifty500\n")

    # Load both databases
    print("  Loading Nifty 50 data...")
    data50  = load_nifty50()
    print(f"  Nifty 50: {data50['Symbol'].nunique()} stocks loaded")

    print("  Loading Nifty 500 data...")
    data500 = load_nifty500()
    print(f"  Nifty 500: {data500['Symbol'].nunique()} stocks loaded\n")

    # Use dates from Nifty 50 as master (shorter, more reliable)
    all_dates = sorted(set(data50["Date"].unique()) & set(data500["Date"].unique()))
    print(f"  Common trading days: {len(all_dates)}\n")

    # Stock universes
    v4_symbols  = data50["Symbol"].unique().tolist()          # all 48 Nifty 50
    v5_symbols  = get_top_n_by_market_cap(data500, 100)       # top 100 Nifty 500
    cc2_symbols = get_top_n_by_market_cap(data500, 100)       # same

    print(f"  V4 universe : {len(v4_symbols)} Nifty 50 stocks")
    print(f"  V5 universe : top 100 Nifty 500 by market cap")
    print(f"  CC2 universe: top 100 Nifty 500 by market cap\n")

    # Market returns for regime detection
    market_ret_50  = data50.groupby("Date")["Daily_Return"].mean()
    market_ret_500 = data500.groupby("Date")["Daily_Return"].mean()
    ...
    v5.buy_basket(v5_symbols, prices500_day1, first_day)
    cc2.buy_basket(cc2_symbols, prices500_day1, first_day)
    ...
    for day_idx, date in enumerate(all_dates):
        ...
        # Regime from Nifty 50 (more reliable signal)
        if day_idx % REGIME_FREQ == 0:
            mr50           = market_ret_50[market_ret_50.index <= date]
            current_regime = detect_regime_fast(mr50)

        # ── CC2: early warning on Nifty 500 ───────────────────────────────
        if cc2.mode == "CASH":
            cc2.days_in_cash += 1

        if day_idx % 5 == 0 and day_idx > VOL_WINDOW + MOMENTUM_WINDOW:
            n_sig, sigs = compute_early_warning(data500, cc2_symbols, date)

            if cc2.mode == "V5" and n_sig >= SIGNALS_TO_EXIT:
                cc2.liquidate_all(prices500, date, reason="early_warning")
                cc2.mode = "CASH"
                cc2.days_in_cash = 0

            elif cc2.mode == "CASH" and n_sig == 0 and cc2.days_in_cash >= COOLDOWN_DAYS:
                cc2.buy_basket(cc2_symbols, prices500, date)
                cc2.mode = "V5"
                cc2.days_in_cash = 0

        cc2_val = cc2.market_value(prices500)
        if cc2.mode == "V5" and cc2.drawdown(cc2_val) >= EMERGENCY_DD_CC2:
            cc2.liquidate_all(prices500, date, reason="emergency_dd")
            cc2.mode = "CASH"
            cc2.days_in_cash = 0

        # ── V4: full active system on Nifty 50 ────────────────────────────
    ...
    return equity_curve, v5, v4, cc2, data50, data500


# ── METRICS + REPORT ──────────────────────────────────────────────────────────

def compute_and_print(equity_curve, v5, v4, cc2, data50, data500):
    equity       = pd.DataFrame(equity_curve).set_index("date")
    equity.index = pd.to_datetime(equity.index)

    def metrics(series, capital):
        tr  = (series.iloc[-1] - capital) / capital
    ...
    print("\n" + "═"*62)
    print("  PORTFOLIO ALLOCATOR V2 — DUAL DATABASE RESULTS")
    print(f"  {BACKTEST_START} to {BACKTEST_END}")
    print(f"  V5 {V5_FRACTION*100:.0f}% Nifty500 | V4 {V4_FRACTION*100:.0f}% Nifty50 | CC2 {CC2_FRACTION*100:.0f}% Nifty500")
    print("="*62)
    ...
    print(f"  Alpha vs Nifty 50   : {tm['ret']-b50_ret*100:>+11.2f}%")
    print(f"  {'─'*46}")
    ...
    print(f"  Nifty 50 Max DD     : {b50_mdd*100:>+11.2f}%")
    print(f"  {'─'*46}")
    print(f"  Total Trades        : {total_trades:>12}")
    print("="*62)
    print("\n  ── Sub-Portfolio Breakdown ──────────────────────────────")
    print(f"  {'Strategy':<22} {'Alloc':>5} {'DB':>6} {'Return':>8} {'Max DD':>8} {'Sharpe':>7} {'Calmar':>7}")
    print(f"  {'─'*66}")
    print(f"  {'V5 Passive top 100':<22} {'40%':>5} {'N500':>6} {v5m['ret']:>+7.2f}% {v5m['mdd']:>+7.2f}% {v5m['sh']:>7.3f} {v5m['cal']:>7.3f}")
    print(f"  {'V4 Active Nifty 50':<22} {'40%':>5} {'N50':>6} {v4m['ret']:>+7.2f}% {v4m['mdd']:>+7.2f}% {v4m['sh']:>7.3f} {v4m['cal']:>7.3f}")
    print(f"  {'CC2 Early Warning':<22} {'20%':>5} {'N500':>6} {c2m['ret']:>+7.2f}% {c2m['mdd']:>+7.2f}% {c2m['sh']:>7.3f} {c2m['cal']:>7.3f}")
    print(f"  {'─'*66}")
    print(f"  {'COMBINED':<22} {'100%':>5} {'BOTH':>6} {tm['ret']:>+7.2f}% {tm['mdd']:>+7.2f}% {tm['sh']:>7.3f} {tm['cal']:>7.3f}")
    print("="*62)

    equity.to_csv("data/equity_curve_allocator_v2.csv")
    all_trades.to_csv("data/trade_log_allocator_v2.csv", index=False)
    print(f"\n  Equity curve → data/equity_curve_allocator_v2.csv")
    print(f"  Trade log    → data/trade_log_allocator_v2.csv")


if __name__ == "__main__":
    print("Portfolio Allocator V2 — Dual Database (Nifty 50 + Nifty 500)")
    equity_curve, v5, v4, cc2, data50, data500 = run_allocator()
    compute_and_print(equity_curve, v5, v4, cc2, data50, data500)

...
backtester_v6.py
----------------
Backtester V6 — 50% Passive Hold + 50% Active V4 System.
 
Strategy:
  - 50% of capital buys top 15 Nifty stocks equally on day 1, never touched
  - 50% of capital runs the full V4 active system (all 9 agents)
  - Passive half provides benchmark-like returns
  - Active half provides drawdown protection and alpha in downturns
  - Target: ~150% return, ~-25% max drawdown
 
Usage:
    python backtester_v6.py
"""
...
# ── V6 PARAMETERS ─────────────────────────────────────────────────────────────
PASSIVE_FRACTION = 0.50   # held in top N stocks forever
ACTIVE_FRACTION  = 0.50   # runs V4 active system
INDEX_TOP_N      = 48     # number of stocks in passive basket
# ──────────────────────────────────────────────────────────────────────────────
...
class BacktestPortfolio:
    # Active portion
    active_cash:     float = INITIAL_CAPITAL * ACTIVE_FRACTION
    active_positions: dict = field(default_factory=dict)
    # Passive portion — bought on day 1, never touched
    passive_holdings: dict = field(default_factory=dict)  # symbol -> shares
    # Shared
    equity_curve:    list  = field(default_factory=list)
    trade_log:       list  = field(default_factory=list)
    peak_value:      float = INITIAL_CAPITAL
 
    def passive_value(self, prices):
        return sum(
            self.passive_holdings[s] * prices.get(s, 0)
            for s in self.passive_holdings
        )
 
    def active_value(self, prices):
        return self.active_cash + sum(
            self.active_positions[s]["shares"] * prices.get(s, 0)
            for s in self.active_positions
        )
 
    def market_value(self, prices):
        return self.passive_value(prices) + self.active_value(prices)
    ...
    def buy(self, symbol, shares, price, date):
        ...
        if cost > self.active_cash:
            shares = int(self.active_cash / (price * (1 + COMMISSION_PCT)))
            cost   = shares * price * (1 + COMMISSION_PCT)
        if shares == 0: return False
        self.active_cash -= cost
        if symbol in self.active_positions:
            old = self.active_positions[symbol]; tot = old["shares"] + shares
            avg = (old["shares"] * old["entry_price"] + shares * price) / tot
            self.active_positions[symbol] = {"shares": tot, "entry_price": avg}
        else:
            self.active_positions[symbol] = {"shares": shares, "entry_price": price}
    ...
    def sell(self, symbol, price, date, reason="signal"):
        if symbol not in self.active_positions: return False
        shares = self.active_positions[symbol]["shares"]
        entry  = self.active_positions[symbol]["entry_price"]
        pnl    = (price - entry) * shares
        self.active_cash += shares * price * (1 - COMMISSION_PCT)
...
def run_backtest_v6(symbol=None):
    print(f"\nV6 Backtest ({BACKTEST_START} to {BACKTEST_END})...")
    ...
    config     = RiskConfig(total_capital=INITIAL_CAPITAL * ACTIVE_FRACTION)
    market_ret = all_data.groupby("Date")["Daily_Return"].mean()
 
    print(f"Backtesting {len(symbols)} stock(s) across {len(all_dates)} days...")
    print(f"  Passive={PASSIVE_FRACTION*100:.0f}% buy & hold | "
          f"Active={ACTIVE_FRACTION*100:.0f}% V4 system\n")
 
    # ── Buy passive basket on day 1 ────────────────────────────────────────
    ...
    passive_cash = INITIAL_CAPITAL * PASSIVE_FRACTION
    per_stock    = passive_cash / INDEX_TOP_N
    ...
    for sym in top_syms:
        ...
        if price and price > 0:
            ...
            cost   = shares * price * (1 + COMMISSION_PCT)
            if shares > 0 and cost <= passive_cash:
                portfolio.passive_holdings[sym] = shares
                passive_cash -= cost
 
    print(f"  Passive basket: {', '.join(top_syms)}\n")
        ...
        if day_idx % REBALANCE_FREQ == 0:
            ...
            if portfolio.drawdown(total_val) > config.max_drawdown_pct:
                ...
                portfolio.equity_curve.append({
                    ...
                    "active_value":    portfolio.active_value(day_prices),
                    "passive_value":   portfolio.passive_value(day_prices),
            ...
            for sym in symbols:
                ...
                if signal > config.min_signal_strength:
                    if sym not in portfolio.active_positions:
                        shares = int(portfolio.active_cash * size_frac / current_price)
                        if shares > 0:
...
def print_report(metrics, portfolio):
    print("\n" + "═" * 58)
    print("  BACKTESTER V6 — HYBRID PERFORMANCE REPORT")
    print(f"  {BACKTEST_START} to {BACKTEST_END}  "
          f"[{int(PASSIVE_FRACTION*100)}% passive + {int(ACTIVE_FRACTION*100)}% active]")
    print("=" * 58)
    ...
    print("=" * 58)
    print("\n  ── Full Comparison ────────────────────────────────")
    print(f"  {'Metric':<22} {'V4':>10} {'V6 Hybrid':>10} {'V5 Passive':>11} {'Benchmark':>11}")
    print(f"  {'─'*66}")
    print(f"  {'Total Return':<22} {'  +66.34%':>10} {metrics['total_return']:>+9.2f}% {'  +204.47%':>11} {'  +176.48%':>11}")
    print(f"  {'Max Drawdown':<22} {'  -16.19%':>10} {metrics['max_drawdown']:>+9.2f}% {'   -40.63%':>11} {metrics['bench_max_dd']:>+10.2f}%")
    print("=" * 58)
 
    eq_df = pd.DataFrame(portfolio.equity_curve)
    eq_df.to_csv("data/equity_curve_v6.csv", index=False)
    print(f"\n  Equity curve → data/equity_curve_v6.csv")
    if portfolio.trade_log:
        pd.DataFrame(portfolio.trade_log).to_csv("data/trade_log_v6.csv", index=False)
        print(f"  Trade log    → data/trade_log_v6.csv")
...
if __name__ == "__main__":
    ...
    if "--symbol" in sys.argv:
        ...
        print(f"Single stock V6: {symbol}")
    else:
        print("Full Nifty 50 V6 backtest...")
 
    portfolio = run_backtest_v6(symbol)

...
backtester_v4.py
----------------
Backtester V4 — all 9 agents including Transformer and RL weighting.

Key additions over V3:
  - Transformer attention signal
  - RL DQN dynamic weight optimization
  - FinBERT sentiment (news — neutral in backtest, no historical news)
  - Heston vol-adjusted position sizing

Usage:
    python backtester_v4.py
    python backtester_v4.py --symbol RELIANCE
"""
...
from risk.risk_engine_v3 import RiskConfig
...
def kalman_velocity(close):
    ...
    for price in close.values:
        ...
        S=float((H@Pp@H.T)[0,0])+R; K=(Pp@H.T)/S; Kf=K.flatten()
        inn=float(price)-float((H@xp)[0]); x=xp+Kf*inn; P=(np.eye(2)-Kf[:,None]*H)@Pp
...
def adf_stat_fast(series):
    ...
    try:
        b=np.linalg.lstsq(X,dy,rcond=None)[0]; res=dy-X@b
...
def lstm_proxy(close,window=60):
    ...
    return float(np.clip(0.6*np.clip(momentum_accel*50,-1,1)+0.4*streak/5,-1,1))


def transformer_proxy(close,window=60):
    """
    Fast transformer proxy using attention-like pattern matching.
    Finds similar historical windows and weights their outcomes.
    """
    ...
    # Find similar historical windows (simplified attention)
    similarities = []
    outcomes     = []
    ...
    for i in range(window, len(returns)-5, 5):
        ...
        # Cosine similarity (attention score)
        sim = float(np.dot(current_norm, hist_norm) / (np.linalg.norm(current_norm)*np.linalg.norm(hist_norm)+1e-8))
...
def rl_dynamic_weights(agent_scores, market_features, episode=0):
    """
    Fast RL proxy — learned weight adjustments based on market state.
    In backtest we simulate the RL learning with a rule-based approximation
    ...
    # High vol → weight heston more
    # Strong trend → weight kalman more
    # Bear regime → weight mean rev more
    if vol > 0.7:
...
def compute_all_signals_v4(sym_data, current_idx, regime=0):
    ...
    sent_score     = 0.0  # neutral in backtest

    # RL dynamic weights
    vol=float(returns.iloc[-60:].std()) if len(returns)>=60 else 0.02
    trend=float(returns.iloc[-60:].mean()) if len(returns)>=60 else 0.0
    market_features={"vol":np.clip(vol*50,0,1),"trend":np.clip(trend*100,-1,1),
                     "regime":regime/2.0,"momentum":0.0}
    agent_scores={"kalman":mom_score,"mean_rev":mr_score,"lstm":lstm_score,
                  "heston":heston_s,"monte_carlo":mc_score,"sentiment":sent_score}
...
class BacktestPortfolio:
    cash:float=INITIAL_CAPITAL; positions:dict=field(default_factory=dict)
    equity_curve:list=field(default_factory=list); trade_log:list=field(default_factory=list)
    peak_value:float=INITIAL_CAPITAL

    def market_value(self,prices):
        return self.cash+sum(self.positions[s]["shares"]*prices.get(s,0) for s in self.positions)
    ...
    def buy(self,symbol,shares,price,date):
        ...
        if cost>self.cash: shares=int(self.cash/(price*(1+COMMISSION_PCT))); cost=shares*price*(1+COMMISSION_PCT)
        ...
        else: self.positions[symbol]={"shares":shares,"entry_price":price}
        self.trade_log.append({"date":date,"symbol":symbol,"action":"BUY","shares":shares,"price":price,"value":shares*price,"pnl":None})
        return True

    def sell(self,symbol,price,date,reason="signal"):
        if symbol not in self.positions: return False
        shares=self.positions[symbol]["shares"]; entry=self.positions[symbol]["entry_price"]
        pnl=(price-entry)*shares; self.cash+=shares*price*(1-COMMISSION_PCT)
        del self.positions[symbol]
        self.trade_log.append({"date":date,"symbol":symbol,"action":"SELL","shares":shares,"price":price,"value":shares*price,"pnl":pnl,"reason":reason})
        return True


def run_backtest_v4(symbol=None):
    print(f"\nV4 Backtest ({BACKTEST_START} to {BACKTEST_END})...")
    all_data=load_data(symbol)
    if all_data.empty: print("No data."); return None

    symbols=all_data["Symbol"].unique(); all_dates=sorted(all_data["Date"].unique())
    portfolio=BacktestPortfolio(); config=RiskConfig(total_capital=INITIAL_CAPITAL)
    ...
    for day_idx,date in enumerate(all_dates):
        ...
        for sym in list(portfolio.positions.keys()):
            ...
            price=day_prices[sym]; entry=portfolio.positions[sym]["entry_price"]
            loss=(price-entry)/entry
            stop=config.stop_loss_pct*{0:1.0,1:0.85,2:0.70}.get(current_regime,1.0)
            if loss<-stop: portfolio.sell(sym,price,date,reason="stop_loss")
        ...
        if day_idx%REBALANCE_FREQ==0:
            ...
            if portfolio.drawdown(total_val)>config.max_drawdown_pct:
                for sym in list(portfolio.positions.keys()):
                    if sym in day_prices: portfolio.sell(sym,day_prices[sym],date,reason="circuit_breaker")
                portfolio.equity_curve.append({"date":date,"portfolio_value":portfolio.market_value(day_prices),
            ...
            for sym in symbols:
                ...
                result=compute_all_signals_v4(sym_data,current_idx,current_regime)
                ...
                size_frac=min(kelly_f*config.kelly_fraction*regime_s*heston_pos_mult,config.max_position_pct)

                if signal>config.min_signal_strength:
                    if sym not in portfolio.positions:
                        shares=int(portfolio.cash*size_frac/current_price)
                        if shares>0: portfolio.buy(sym,shares,current_price,date)
        ...
        portfolio.equity_curve.append({"date":date,"portfolio_value":total_value,
            "cash":portfolio.cash,"n_positions":len(portfolio.positions),"regime":current_regime})
...
def compute_metrics(portfolio,all_data):
    ...
    rf_daily=0.06/252; excess=daily_ret-rf_daily
    ...
    return {
        "initial_capital":INITIAL_CAPITAL,"final_value":round(final_val,2),
        "total_return":round(total_ret*100,2),"annual_return":round(annual*100,2),
        ...
        "calmar_ratio":round(calmar,3),"max_drawdown":round(max_dd*100,2),
        "total_trades":len(trades),"win_rate":round(win_rate*100,2),
...
def print_report(metrics,portfolio):
    print("\n"+"═"*58)
    print("  BACKTESTER V4 — PERFORMANCE REPORT")
    ...
    print(f"  Alpha               : {metrics['alpha']:>+11.2f}%  ({'outperformed' if metrics['alpha']>0 else 'underperformed'})")
    print(f"  {'─'*42}")
...
if __name__=="__main__":
    ...
    if "--symbol" in sys.argv:
        idx=sys.argv.index("--symbol"); symbol=sys.argv[idx+1].upper()
        print(f"Single stock V4: {symbol}")
    else:
        print("Full Nifty 50 V4 backtest...")
    portfolio=run_backtest_v4(symbol)
